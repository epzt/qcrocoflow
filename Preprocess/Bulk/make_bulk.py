...
def process_files(era5_dir, y_min, y_max, m_min, m_max, grid_dir, title, grdname, Dmin, Yorig, Mmin):
    """
    Process all files in the given directory
    """
    # Open the croco grid file
    croco_grid = xr.open_dataset(os.path.join(grid_dir, 'croco_grd.nc'))
    ...
    for Y in range(y_min, y_max + 1):
        ...
        for M in range(mo_min, mo_max + 1):
            ...
            create_bulk(blkname, grdname, title, time, 0)
            add_global_attributes(blkname, Yorig, Mmin, Dmin, 'ERA5')
            interp_ERA5(era5_dir, Y, M, blkname, angle, tlen0)
            print(" ")
...
def main():
    ...
    d_min = 1
    d_max = 2
    grid_dir = "C:\\Users\\johnn\\Desktop\\grid"
    era5_dir = "C:\\Users\\johnn\\Desktop\\Nouveau dossier"
    title = "croco_bulk"
    grdname = "C:\\Users\\johnn\\Desktop\\grid\\croco_grd.nc"

    y_min, y_max, m_min, m_max = get_y_m(era5_dir)
    process_files(era5_dir, y_min, y_max, m_min, m_max, grid_dir, title, grdname, d_min, y_orig, mo_min )
